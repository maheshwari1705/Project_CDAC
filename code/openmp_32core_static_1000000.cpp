...
#define THREADS 32         // Number of OpenMP threads

// Updated static chunk size
#define STATIC_CHUNK_SIZE 1000000
...
void blackScholesPDE(float* optionPrices, int numSteps, float Smax, float dS, float dt, float r, float sigma, float K) {
    ...
    #pragma omp parallel for num_threads(THREADS) schedule(static, STATIC_CHUNK_SIZE)
